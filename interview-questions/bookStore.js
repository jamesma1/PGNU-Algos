...
** Question for the first 1hr technical with TheRealReal (Sr. Frontend Position) **
...
5 books at a 25% discount + 3 books at a 10% discount. Resulting in:
...
const bookStore = (basket) => {
  ...
  const quantities = {};
  basket.forEach((book) => {
    quantities[book] ? quantities[book]++ : (quantities[book] = 1);
  });

  const numSets = Math.max(Object.values(quantities));
